#ifndef CACHYOS_GUI_INSTALLER_BACKEND_HPP
#define CACHYOS_GUI_INSTALLER_BACKEND_HPP

#include "LogModel.hpp"

// import instlib
#include "cachyos/partition_planner.hpp"
#include "cachyos/requirements.hpp"
#include "cachyos/types.hpp"

#include <optional>
#include <string>
#include <vector>

#include <QHash>
#include <QObject>
#include <QStringList>
#include <QThread>
#include <QUrl>
#include <QVariantList>
#include <QVariantMap>
#include <QtQmlIntegration>

class InstallWorker;

class InstallerBackend : public QObject {
    Q_OBJECT
    QML_ELEMENT
    QML_SINGLETON

    // Navigation
    Q_PROPERTY(int32_t currentPage READ currentPage WRITE setCurrentPage NOTIFY currentPageChanged)
    Q_PROPERTY(int32_t pageCount READ pageCount CONSTANT)
    Q_PROPERTY(bool canGoNext READ canGoNext NOTIFY canGoNextChanged)
    Q_PROPERTY(bool canGoBack READ canGoBack NOTIFY canGoBackChanged)

    // System state
    Q_PROPERTY(bool isUefi READ isUefi NOTIFY systemInfoChanged)
    Q_PROPERTY(bool isConnected READ isConnected NOTIFY systemInfoChanged)
    Q_PROPERTY(bool isRoot READ isRoot CONSTANT)

    // Welcome-page requirements (registry-driven; see cachyos/requirements.hpp)
    Q_PROPERTY(QVariantList requirements READ requirements NOTIFY requirementsChanged)
    Q_PROPERTY(bool mandatoryRequirementsSatisfied READ mandatoryRequirementsSatisfied NOTIFY requirementsChanged)

    // Data lists
    Q_PROPERTY(QStringList deviceList READ deviceList NOTIFY deviceListChanged)
    Q_PROPERTY(QStringList filesystemList READ filesystemList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList kernelList READ kernelList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList desktopList READ desktopList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList bootloaderList READ bootloaderList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList shellList READ shellList NOTIFY listsPopulated)
    Q_PROPERTY(QVariantList bootloaderOptions READ bootloaderOptions NOTIFY listsPopulated)
    Q_PROPERTY(QStringList localeList READ localeList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList keymapList READ keymapList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList timezoneList READ timezoneList NOTIFY listsPopulated)
    Q_PROPERTY(QStringList forbiddenLogins READ forbiddenLogins CONSTANT)
    Q_PROPERTY(QStringList forbiddenHostnames READ forbiddenHostnames CONSTANT)

    // Install progress
    Q_PROPERTY(bool isInstalling READ isInstalling NOTIFY isInstallingChanged)
    Q_PROPERTY(bool isCancelling READ isCancelling NOTIFY isCancellingChanged)
    Q_PROPERTY(double installProgress READ installProgress NOTIFY installProgressChanged)
    Q_PROPERTY(QString installStatus READ installStatus NOTIFY installStatusChanged)
    Q_PROPERTY(QString errorMessage READ errorMessage NOTIFY errorMessageChanged)
    // Append-only model of install-log lines (see LogModel).
    Q_PROPERTY(QAbstractListModel* logModel READ logModel CONSTANT)

    // Partition planner
    Q_PROPERTY(QString plannerError READ plannerError NOTIFY plannerErrorChanged)
    Q_PROPERTY(bool hasMountSelections READ hasMountSelections NOTIFY mountSelectionsChanged)

 public:
    explicit InstallerBackend(QObject* parent = nullptr);
    ~InstallerBackend() override;

    // Forward a log line (e.g. from a spdlog sink on any thread) to the live
    // GUI log view. Thread-safe and a no-op until a backend instance exists.
    static void forwardLogLine(const std::string& message);

    [[nodiscard]] QAbstractListModel* logModel() const { return m_logModel; }

    // Navigation
    [[nodiscard]] int32_t currentPage() const { return m_currentPage; }
    void setCurrentPage(int32_t page);
    [[nodiscard]] int32_t pageCount() const { return 11; }
    [[nodiscard]] bool canGoNext() const;
    [[nodiscard]] bool canGoBack() const;

    // System state
    [[nodiscard]] bool isUefi() const { return m_isUefi; }
    [[nodiscard]] bool isConnected() const { return m_isConnected; }
    [[nodiscard]] bool isRoot() const { return m_isRoot; }

    [[nodiscard]] QVariantList requirements() const { return m_requirements; }
    [[nodiscard]] bool mandatoryRequirementsSatisfied() const { return m_mandatoryRequirementsSatisfied; }

    // Data lists
    [[nodiscard]] QStringList deviceList() const { return m_deviceList; }
    [[nodiscard]] QStringList filesystemList() const { return m_filesystemList; }
    [[nodiscard]] QStringList kernelList() const { return m_kernelList; }
    [[nodiscard]] QStringList desktopList() const { return m_desktopList; }
    [[nodiscard]] QStringList bootloaderList() const { return m_bootloaderList; }
    [[nodiscard]] QStringList shellList() const { return m_shellList; }
    [[nodiscard]] QVariantList bootloaderOptions() const { return m_bootloaderOptions; }
    [[nodiscard]] QStringList localeList() const { return m_localeList; }
    [[nodiscard]] QStringList keymapList() const { return m_keymapList; }
    [[nodiscard]] QStringList timezoneList() const { return m_timezoneList; }
    [[nodiscard]] static QStringList forbiddenLogins();
    [[nodiscard]] static QStringList forbiddenHostnames();

    // Install progress
    [[nodiscard]] bool isInstalling() const { return m_isInstalling; }
    [[nodiscard]] bool isCancelling() const { return m_isCancelling; }
    [[nodiscard]] double installProgress() const { return m_installProgress; }
    [[nodiscard]] QString installStatus() const { return m_installStatus; }
    [[nodiscard]] QString errorMessage() const { return m_errorMessage; }

    // Partition planner
    [[nodiscard]] QString plannerError() const { return m_plannerError; }
    [[nodiscard]] bool hasMountSelections() const { return m_mountSelections.has_value(); }

    Q_INVOKABLE void initialize();
    Q_INVOKABLE void refreshDevices();
    Q_INVOKABLE void refreshRequirements();
    Q_INVOKABLE QStringList getAvailableMountOpts(const QString& fstype) const;
    // Asynchronously resolve the packages installed for a desktop environment
    // from net-profiles (network with file fallback) on a worker thread, so the
    // GUI never blocks on I/O. The result arrives via desktopPackagesReady().
    // Fetches are single-flight + coalesced: requests made while one is running
    // are deferred and only the latest is honoured, which also keeps the
    // (unsynchronised) net-profiles cache free of concurrent writers.
    Q_INVOKABLE void fetchDesktopPackages(const QString& desktop);
    // Asynchronously resolve the optional "netinstall" package groups from
    // net-profiles on a worker thread (same cache as fetchDesktopPackages). The
    // result — a recursive tree of group maps — arrives via netinstallGroupsReady().
    // Coalesced single-flight like the desktop fetch.
    Q_INVOKABLE void fetchNetinstallGroups();
    // Validates the gathered selections and, if complete, spawns the install
    // worker. Returns false (without starting anything) when a required field is
    // missing.
    Q_INVOKABLE bool startInstallation(const QVariantMap& selections);
    Q_INVOKABLE void cancelInstallation();

    // Reboots the host into the freshly installed system. Returns false if the
    // reboot command could not be dispatched (the caller stays in the live env).
    Q_INVOKABLE bool reboot() const;

    // Partition planner bridges. All "QVariantMap" / "QVariantList" payloads
    // mirror the structs in cachyos::installer::partition_planner so QML
    // never has to import planner headers.
    Q_INVOKABLE QVariantMap discoverDisks();
    Q_INVOKABLE QStringList inspectExistingBtrfs(const QString& device);
    // Unallocated free regions on a disk, as [{ startBytes, endBytes, sizeBytes }]
    // for the "install alongside" flow. Non-destructive query.
    Q_INVOKABLE QVariantList freeSpaceRegions(const QString& device);
    // Creates a root partition filling [startBytes, endBytes] in a disk's free
    // space (existing partitions untouched) and returns its device path, or an
    // empty string on failure (plannerError carries the reason).
    Q_INVOKABLE QString createAlongsidePartition(const QString& device,
        qulonglong startBytes, qulonglong endBytes);
    // Existing partitions on a disk as [{ number, startBytes, endBytes, sizeBytes,
    // device, fstype, name, flags }].
    Q_INVOKABLE QVariantList partitionsForDevice(const QString& device);
    // Shrink range for a partition as { min, max } bytes (empty map on failure).
    Q_INVOKABLE QVariantMap shrinkableBounds(const QString& device, uint number,
        const QString& fstype, qulonglong currentSizeBytes);
    // Applies an ordered list of partition operations (each a QVariantMap with a
    // "kind" plus the relevant fields); returns the last-created partition path or
    // "" on failure (plannerError carries the reason).
    Q_INVOKABLE QString applyPartitionOps(const QString& device, const QVariantList& ops);
    // Read-only: records the destructive partition ops + mount rows to apply at
    // install time (on the worker thread, as root). "Validate plan" calls this
    // instead of touching the disk. `mounts` are [{ source:"existing"|"created",
    // device, mountpoint, fstype, formatRequested, mountOpts }]. Returns false and
    // sets plannerError when the plan has no "/" mount.
    Q_INVOKABLE bool stagePartitionPlan(const QString& device,
        const QVariantList& ops, const QVariantList& mounts);
    // Drops any staged partition plan (e.g. when the user switches disk/mode).
    Q_INVOKABLE void clearStagedPartitioning();
    // Human-readable lines describing the staged partition ops + mounts, for the
    // Summary page. Empty when nothing is staged.
    Q_INVOKABLE QStringList stagedPartitionSummary() const;
    Q_INVOKABLE QVariantList defaultBtrfsLayout() const;
    Q_INVOKABLE QVariantList defaultZfsLayout(const QString& zpool) const;
    Q_INVOKABLE QString suggestMountpointForSubvolume(const QString& subvol) const;
    Q_INVOKABLE bool prepareZpool(const QString& partition, const QString& zpool, const QString& mountpoint);
    // Stages a whole-disk ZFS root install (applied on the worker thread at
    // install time: erases `device`, creates the default zpool/datasets on it,
    // optionally with ZFS native encryption). Clears any other staged plan.
    Q_INVOKABLE bool stageZfsRoot(const QString& device, const QString& zpool,
        bool encrypt, const QString& passphrase);
    // Stages the default/auto whole-disk layout (Erase disk).
    Q_INVOKABLE bool stageAutoLayout(const QString& device, const QString& fs,
        bool encrypt, const QString& passphrase, bool useLuks2 = true);
    Q_INVOKABLE bool encryptPartition(const QString& device, const QString& mapperName,
        const QString& passphrase, const QString& extraFlags,
        const QString& luksVersion = QStringLiteral("Luks2"));
    Q_INVOKABLE bool openEncryptedPartition(const QString& device, const QString& mapperName,
        const QString& passphrase);
    Q_INVOKABLE bool finalizePlan(const QVariantMap& plan);
    Q_INVOKABLE void clearMountSelections();
    // True only when `source` is a genuinely animated image (more than one frame).
    Q_INVOKABLE bool isAnimated(const QUrl& source) const;

 signals:
    void currentPageChanged();
    void canGoNextChanged();
    void canGoBackChanged();
    void systemInfoChanged();
    void deviceListChanged();
    void listsPopulated();
    void isInstallingChanged();
    void isCancellingChanged();
    void installProgressChanged();
    void installStatusChanged();
    void errorMessageChanged();
    void installFinished(bool success, const QString& error);
    void desktopPackagesReady(const QString& desktop, const QStringList& packages);
    void netinstallGroupsReady(const QVariantList& groups);
    void plannerErrorChanged();
    void mountSelectionsChanged();
    void requirementsChanged();

 private slots:
    void onWorkerProgress(double fraction, const QString& step);
    void onWorkerFinished(bool success, const QString& error);

 private:
    void populateStaticLists();

    // Kicks off the next queued desktop-package fetch on the global thread pool
    // (no-op when one is already running or nothing is pending).
    void startNextDesktopFetch();
    // Does the actual net-profiles resolve; runs on a worker thread.
    static QStringList computeDesktopPackages(const QString& desktop);

    bool m_pkgFetchBusy{false};
    bool m_pkgFetchHasPending{false};
    QString m_pkgFetchPending;

    // Kicks off the netinstall-groups fetch on the global thread pool (no-op when
    // one is already running; coalesced so the cache has no concurrent writers).
    void startNetinstallFetch();
    // Resolves + converts the netinstall group tree to a QVariantList; worker thread.
    static QVariantList computeNetinstallGroups();

    bool m_netFetchBusy{false};
    bool m_netFetchHasPending{false};

    // Returns an empty string when `selections` carry every field needed to
    // start an install, otherwise a human-readable description of what's missing.
    [[nodiscard]] QString validateSelections(const QVariantMap& selections) const;

    int32_t m_currentPage{0};
    bool m_isUefi{false};
    bool m_isConnected{false};
    bool m_isRoot{false};
    bool m_isInstalling{false};
    bool m_isCancelling{false};
    double m_installProgress{0.0};
    QString m_installStatus;
    QString m_errorMessage;

    QStringList m_deviceList;
    QStringList m_filesystemList;
    QStringList m_kernelList;
    QStringList m_desktopList;
    QStringList m_bootloaderList;
    QStringList m_shellList;
    QVariantList m_bootloaderOptions;
    QStringList m_localeList;
    QStringList m_keymapList;
    QStringList m_timezoneList;

    QString m_plannerError;
    std::optional<cachyos::installer::MountSelections> m_mountSelections;

    // Partition edits staged at validate time, applied on the worker thread at
    // install time. Empty unless a Manual/Alongside plan needs destructive ops.
    [[nodiscard]] bool hasStagedPartitioning() const {
        return !m_stagedPartOps.empty() || !m_stagedPartMounts.empty();
    }
    QString m_stagedPartDevice;
    std::vector<cachyos::installer::partition_planner::PartitionOp> m_stagedPartOps;
    std::vector<cachyos::installer::partition_planner::StagedMount> m_stagedPartMounts;

    // Whole-disk ZFS root staged at validate time, applied on the worker thread.
    bool m_stagedZfsActive{false};
    QString m_stagedZfsDevice;
    QString m_stagedZfsPool;
    bool m_stagedZfsEncrypt{false};
    QString m_stagedZfsPassphrase;

    // Whole-disk default/auto layout ("Erase disk") staged at validate time. When
    // active, startInstallation resolves the strategy to EraseAndAuto{device} so the
    // orchestrator actually partitions/formats the disk (optionally LUKS root).
    bool m_stagedAutoActive{false};
    QString m_stagedAutoDevice;
    QString m_stagedAutoFs;
    bool m_stagedAutoEncrypt{false};
    QString m_stagedAutoPassphrase;
    bool m_stagedAutoLuks2{true};

    QVariantList m_requirements;
    bool m_mandatoryRequirementsSatisfied{false};

    // Memoises isAnimated() results by source URL (see isAnimated()).
    mutable QHash<QString, bool> m_animatedCache;

    // Owns the install-log lines shown in the InstallPage ListView. Child of `this`.
    LogModel* m_logModel{nullptr};

    QThread m_workerThread;
    InstallWorker* m_worker{nullptr};

    void setPlannerError(QString error);
};

#endif  // CACHYOS_GUI_INSTALLER_BACKEND_HPP
