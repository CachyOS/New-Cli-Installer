...
void InstallerBackend::forwardLogLine(const std::string& message) {
    ...
    // Logging can happen on the install worker thread. The model
    // append must run where the model lives (the GUI thread).
    const auto line = QString::fromStdString(message);
    QMetaObject::invokeMethod(
        inst, [inst, line]() { inst->m_logModel->appendLine(line); }, Qt::QueuedConnection);
}

InstallerBackend::InstallerBackend(QObject* parent)
  : QObject(parent), m_isRoot(cachyos::installer::check_root()), m_logModel(new LogModel(this)) {
...
bool InstallerBackend::startInstallation(const QVariantMap& selections) {
    ...
    if (m_mountSelections.has_value()) {
        ctx.strategy = cachyos::installer::partition_strategy::ApplyLayout{
            .selections = std::move(*m_mountSelections),
        };
        m_mountSelections.reset();
    } else if (m_stagedAutoActive) {
        ctx.strategy = cachyos::installer::partition_strategy::EraseAndAuto{
            .device = m_stagedAutoDevice.toStdString(),
        };
        if (!m_stagedAutoFs.isEmpty()) {
            ctx.filesystem_name = m_stagedAutoFs.toStdString();
        }
        if (m_stagedAutoEncrypt) {
            ctx.root_luks_passphrase = m_stagedAutoPassphrase.toStdString();
            ctx.root_luks_use_luks2  = m_stagedAutoLuks2;
        }
    }
    ...
    // Full-disk LUKS encryption for the default auto layout is carried by the staged auto plan.
...
bool InstallerBackend::stageAutoLayout(const QString& device, const QString& fs,
    bool encrypt, const QString& passphrase, bool useLuks2) {
    setPlannerError({});
    if (device.isEmpty()) {
        setPlannerError(QStringLiteral("pick a target device to erase for the default layout"));
        return false;
    }
    if (encrypt && passphrase.isEmpty()) {
        setPlannerError(QStringLiteral("enter a LUKS passphrase for the encrypted root"));
        return false;
    }

    // drop any other staged plan
    clearStagedPartitioning();
    clearMountSelections();
    m_stagedAutoActive     = true;
    m_stagedAutoDevice     = device;
    m_stagedAutoFs         = fs;
    m_stagedAutoEncrypt    = encrypt;
    m_stagedAutoPassphrase = encrypt ? passphrase : QString{};
    m_stagedAutoLuks2      = useLuks2;
    return true;
}
...
void InstallerBackend::clearStagedPartitioning() {
    ...
    m_stagedAutoActive = false;
    m_stagedAutoDevice.clear();
    m_stagedAutoFs.clear();
    m_stagedAutoEncrypt = false;
    m_stagedAutoPassphrase.clear();
    m_stagedAutoLuks2 = true;
}
...
bool InstallerBackend::isAnimated(const QUrl& source) const {
    const QString key = source.toString();

    // QML re-evaluates the carousel's animation binding on every
    // current-card/visibility change, and probing the image each time would re-open the
    // decoder repeatedly.
    if (const auto it = m_animatedCache.constFind(key); it != m_animatedCache.constEnd()) {
        return it.value();
    }

    // map the QML source URL to something QImageReader can open.
    QString path;
    if (source.scheme() == QLatin1String("qrc")) {
        path = QLatin1Char(':') + source.path();
    ...
    bool animated = false;
    // never open formats that cannot animate here.
    const QString lower = path.toLower();
    const bool probe    = lower.endsWith(QLatin1String(".webp"))
        || lower.endsWith(QLatin1String(".gif"))
        || lower.endsWith(QLatin1String(".apng"))
        || lower.endsWith(QLatin1String(".mng"));
    if (probe) {
        QImageReader reader(path);
        animated = reader.supportsAnimation() && reader.imageCount() > 1;
    }

    m_animatedCache.insert(key, animated);
    return animated;
}
