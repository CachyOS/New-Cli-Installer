...
Item {
    ...
    function buildPlan() {
        ...
        // Wizard mode (default layout, whole disk): stage an auto/erase layout so the
        // orchestrator resolves the strategy to EraseAndAuto and actually partitions
        // the disk. (Merely clearing mount_selections leaves the strategy at its
        // UseExisting default, which skips disk prep and pacstraps into RAM.)
        if (root.useDefaultLayout) {
            if (root.encryptRoot && root.luksPassphrase.length === 0) {
                root.statusMessage = "LUKS passphrase is required.";
                return false;
            }
            var aok = InstallerBackend.stageAutoLayout(
                root.selectedDevice, root.selectedFs,
                root.encryptRoot, root.encryptRoot ? root.luksPassphrase : "", true);
            if (!aok) {
                root.statusMessage = "Plan invalid: " + InstallerBackend.plannerError;
                return false;
            }
            root.statusMessage = "Erase disk — a default " + root.selectedFs
                + " layout will be created during install."
