            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        // Static images render through a plain Image (decoded once);
                        // only a genuinely animated source that is the centered card of a
                        // visible carousel gets an AnimatedImage. This avoids driving a
                        // static image through QMovie, which otherwise re-runs the (very
                        // thread-heavy) JPEG-XL decoder in a tight loop.
                        Loader {
                            id: imgLoader
                            anchors.fill: parent
                            anchors.margins: Style.marginXS

                            readonly property url imgSource:
                                cardDelegate.modelData[root.imageRole] || root.fallbackImage
                            // Probe the source once (depends only on imgSource); the
                            // play decision then layers on the cheap visibility state.
                            readonly property bool sourceIsAnimated:
                                InstallerBackend.isAnimated(imgSource)
                            readonly property bool wantAnimation:
                                sourceIsAnimated && cardDelegate.isCurrent && root.visible

                            sourceComponent: wantAnimation ? animatedImageComp : staticImageComp

                            Component {
                                id: staticImageComp
                                Image {
                                    source: imgLoader.imgSource
                                    fillMode: Image.PreserveAspectFit
                                    asynchronous: true
                                    smooth: true
                                    cache: true
                                    // Cap decode resolution to the on-screen card size
                                    // so full-res screenshots don't sit in RAM at native
                                    // (often 1080p) size.
