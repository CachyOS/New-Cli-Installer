            ...
            onClicked: {
                if (InstallerBackend.currentPage === 8) {
                    var selections = {
                        ...
                        "kernel":              desktopPage.selectedKernel,
                        "desktop":             desktopPage.selectedDesktop,
                        "excludedPackages":    desktopPage.excludedPackages,
                        "additionalPackages":  packagesPage.additionalPackages,
                        ...
                        "encryptRoot":         partitionPage.encryptRoot,
                        "luksPassphrase":      partitionPage.luksPassphrase
                    };
                    // Only advance to the Install page if the backend accepted
                    // the selections; otherwise stay on Summary so the user can
                    // see the validation error and fix the missing field.
                    if (InstallerBackend.startInstallation(selections))
