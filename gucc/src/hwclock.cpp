...
auto set_hwclock_utc(std::string_view root_mountpoint) noexcept -> Result<void> {
    ...
    return make_error(ErrorCode::SubprocessFailed, fmt::format("hwclock: failed to set UTC clock on '{}'", root_mountpoint));
}
...
auto set_hwclock_localtime(std::string_view root_mountpoint) noexcept -> Result<void> {
    ...
    return make_error(ErrorCode::SubprocessFailed, fmt::format("hwclock: failed to set localtime clock on '{}'", root_mountpoint));
}
