...
TEST_CASE("crypto_detection")
{
    ...
    SECTION("detect_crypto_for_device test")
    {
        SECTION("LUKS alone")
        {
    ...
    SECTION("detect_crypto_for_mountpoint test")
    {
        const std::vector<BlockDevice> devices{
    ...
    SECTION("detect_crypto_for_boot test")
    {
    ...
    SECTION("list_mounted_devices test")
    {
        const std::vector<BlockDevice> devices{
            make_device("/dev/sda1", "part", "vfat", "efi-uuid", std::nullopt, "/mnt/boot/efi"s),
            make_device("/dev/sda2", "part", "ext4", "root-uuid", std::nullopt, "/mnt"s),
    ...
    SECTION("find_underlying_partition test")
    {
        SECTION("LVM chain")
        {
            const std::vector<BlockDevice> devices{
