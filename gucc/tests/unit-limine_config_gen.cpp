...
static constexpr auto LIMINE_CONFIG_SUBVOL_TEST = R"(ESP_PATH="/boot/efi"
KERNEL_CMDLINE[default]="quiet splash rw rootflags=subvol=/@ root=UUID=6bdb3301-8efb-4b84-b0b7-4caeef26fd6f"
BOOT_ORDER="*, *lts, *fallback, Snapshots"
)"sv;
...
TEST_CASE("limine config gen test")
{
    ...
    SECTION("btrfs with subvolumes")
    {
        ...
        const auto& limine_config_text = gucc::bootloader::gen_limine_entry_config(kernel_params, "/boot/efi"sv);
    ...
    SECTION("basic xfs")
    {
        ...
        const auto& limine_config_text = gucc::bootloader::gen_limine_entry_config(kernel_params);
